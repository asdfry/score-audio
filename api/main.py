...
def compute(use_demucs: bool = Query(False), upload_file: UploadFile = File(...)):
    ...
    if use_demucs:  # 음원분리 사용 시
        ...
        accom_path = "/audio/accom.wav"
        separate_result = demucs.separate("input_audio", vocal_path, accom_path)  # 음원 분리
        ...
        # vocal
        audio_path_v = spice.convert_audio_for_model(vocal_path)  # spice에 넣기 위해 준비 (프레임레이트: 16000, 채널: 1, 형식: wav)
        shutil.copy(audio_path_v, "/audio/result.wav")
        sample_rate, audio_samples = wavfile.read(audio_path_v, "rb")
        audio_inform = spice.get_information(sample_rate, audio_samples)
        score = spice.get_confidence_score(audio_samples)  # spice

        # accompaniment
        audio_path_a = spice.convert_audio_for_model(accom_path)  # spice에 넣기 위해 준비 (프레임레이트: 16000, 채널: 1, 형식: wav)
        shutil.copy(audio_path_a, "/audio/accom.wav")
        sample_rate, audio_samples = wavfile.read(audio_path_a, "rb")
        spice.save_spectrogram(audio_samples)
